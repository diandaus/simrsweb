  ...
  <AppLayout>
    <div class="container-fluid">
      <div class="page-header mb-4">
        <h2 class="page-title">🏠 Rawat Inap - Sudah Pulang</h2>
        <p class="page-subtitle">Daftar Pasien Rawat Inap yang Sudah Pulang</p>
      </div>
          ...
          <div class="row g-2 align-items-end">
            ...
            <div class="col-auto">
              <div class="btn-group btn-group-sm" role="group">
                <input type="radio" class="btn-check" name="filterMode" id="radio1" value="tgl_masuk" v-model="filterMode" autocomplete="off">
                <label class="btn btn-outline-success" for="radio1">Tgl Masuk</label>
                <input type="radio" class="btn-check" name="filterMode" id="radio2" value="tgl_keluar" v-model="filterMode" autocomplete="off">
                <label class="btn btn-outline-success" for="radio2">Tgl Keluar</label>
              </div>
            </div>

            <!-- Date Range for Tgl Masuk -->
            <div class="col-auto" v-if="filterMode === 'tgl_masuk'">
            ...
            <!-- Date Range for Tgl Keluar -->
            <div class="col-auto" v-if="filterMode === 'tgl_keluar'">
              <input type="date" class="form-control form-control-sm" v-model="tglKeluarStart" style="width: 140px;">
            </div>
            <div class="col-auto" v-if="filterMode === 'tgl_keluar'">
              <span class="text-muted">s/d</span>
            </div>
            <div class="col-auto" v-if="filterMode === 'tgl_keluar'">
              <input type="date" class="form-control form-control-sm" v-model="tglKeluarEnd" style="width: 140px;">
            </div>
            ...
            <div class="col-auto">
              <select class="form-select form-select-sm" v-model="statusBayarFilter" style="width: 120px;">
            ...
            <div class="col-auto">
              <button class="btn btn-success btn-sm" @click="loadData" :disabled="loading">
      ...
      <div v-else class="card">
        <div class="card-header bg-light d-flex justify-content-between align-items-center">
          <span class="fw-bold">
            ...
            Data Pasien Sudah Pulang
          </span>
          <span class="badge bg-success">{{ dataList.length }} pasien</span>
        </div>
              ...
              <tbody>
                ...
                <template v-else v-for="(item, index) in dataList" :key="item.no_rawat">
                  ...
                  <tr :class="getRowClass(item.stts_pulang)">
                    ...
                    <td>
                      <span class="badge" :class="getStatusPulangClass(item.stts_pulang)">
                        {{ item.stts_pulang }}
                      </span>
                    </td>
...
// Filters
const filterMode = ref('tgl_keluar')
const tglMasukStart = ref(getDate30DaysAgo())
const tglMasukEnd = ref(getTodayDate())
const tglKeluarStart = ref(getDate30DaysAgo())
const tglKeluarEnd = ref(getTodayDate())
...
const loadData = async () => {
  ...
  try {
    ...
    if (filterMode.value === 'tgl_masuk') {
      params.tgl_masuk_start = tglMasukStart.value
      params.tgl_masuk_end = tglMasukEnd.value
    } else if (filterMode.value === 'tgl_keluar') {
      params.tgl_keluar_start = tglKeluarStart.value
      params.tgl_keluar_end = tglKeluarEnd.value
    }

    const { data } = await apiClient.get('/ranap', { params })

    if (data.success) {
      // Filter only patients who already pulang (stts_pulang != '-')
      dataList.value = data.data.filter(item => item.stts_pulang !== '-')
    } else {
      Swal.fire({
        icon: 'error',
        title: 'Gagal',
...
// Reset Filter
const resetFilter = () => {
  filterMode.value = 'tgl_keluar'
  tglMasukStart.value = getDate30DaysAgo()
  tglMasukEnd.value = getTodayDate()
  tglKeluarStart.value = getDate30DaysAgo()
  tglKeluarEnd.value = getTodayDate()
  bangsalFilter.value = ''
...
// Get row class based on status pulang
const getRowClass = (stts_pulang) => {
  if (stts_pulang === 'Meninggal') return 'table-danger'
  return ''
}

// Get badge class for status pulang
const getStatusPulangClass = (stts_pulang) => {
  switch (stts_pulang) {
